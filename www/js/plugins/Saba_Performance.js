...
(function (Saba) {
    ...
    var _Window_Base_prototype_destroyAndRemoveChildren = Window_Base.prototype.destroyAndRemoveChildren;
    Window_Base.prototype.destroyAndRemoveChildren = function () {
        this.contents.returnCanvas();
        _Window_Base_prototype_destroyAndRemoveChildren.call(this);
    };
    ...
    if (parseInt(parameters['notDrawAtBitmapSnap'])) {
        ...
        Bitmap.snap3 = function (stage) {
            ...
            if (stage) {
                Graphics._renderer.render(stage, renderTexture);
                return Graphics._renderer.extract.canvas(renderTexture);
                // context.drawImage(canvas, 0, 0);
            } else {
                //TODO: Ivan: what if stage is not present?
            }

            function roundUpSquareSize(n) {
                var square = 1;
                while (n > square) {
                    square *= 2;
                }
    ...
    if (parseInt(parameters['recycleCanvas'])) {
        ...
        Bitmap.prototype._createCanvas = function (width, height) {
            if (!this.__canvas) {
                if (!width) {
                    if (this._image) {
                        width = Math.max(this._image.width || 0, 1);
                        height = Math.max(this._image.height || 0, 1);
                    }
                }
                if (!width) {
                    ...
                    var cache = getCanvasCache(Math.max(width || 0, 1), Math.max(height || 0, 1));
                    this.__canvas = cache._canvas;
                    this.__context = cache._context;
                }
            } else {
                this.__context = this.__canvas.getContext('2d');
    ...
    if (parseInt(parameters['usePixiSpriteToDrawWindow_Base'])) {
        ...
        Window.prototype.getBaseTexture = function () {
            ...
            if (!baseTexture) {
                baseTexture = new PIXI.BaseTexture(this._windowskin._image, PIXI.settings.SCALE_MODES);
    ...
    if (parseInt(parameters['useFilterToChangePictureTone'])) {
        ...
        var _Sprite_prototype_destroy = Sprite.prototype.destroy;
        Sprite.prototype.destroy = function (a) {
            _Sprite_prototype_destroy.call(this);
            if (this._toneFilter) {
                this._toneFilter = null;
                this._colorTone = null;
            }
        };
    }
    ...
    if (parseInt(parameters['usePixiSpriteToDrawIcon'])) {
         ...
         */
        Window_Base.prototype.drawIcon3 = Window_Base.prototype.drawIcon;
        ...
        Sprite_Base.prototype.drawIcon = function (iconIndex, x, y) {
            var baseTexture = PIXI.utils.BaseTextureCache['system/IconSet'];
            if (!baseTexture) {
                var bitmap = ImageManager.loadSystem('IconSet');
                if (!bitmap.isReady()) {
            ...
            sprite.position.x = x;
            sprite.position.y = y;
            sprite.alpha = this.bitmap.paintOpacity / 255.0;
    ...
    var contentsCacheMap = {};

    function getContentsCache(width, height) {
        var key = width + '_' + height;
        contentsCacheMap[key] = contentsCacheMap[key] || [];
        if (contentsCacheMap[key].length > 0) {
            return contentsCacheMap[key].pop();
        }
        return null;
    }

    function putContentsCache(bitmap, w, h) {
        var key = w + '_' + h;
        //p(key)
        contentsCacheMap[key] = contentsCacheMap[key] || [];
        contentsCacheMap[key].push(bitmap);
    }
    Saba.getContentsCache = getContentsCache;
    Saba.putContentsCache = putContentsCache;
    ...
    if (parseInt(parameters['usePixiByWindow_BattleLogBg'])) {
        ...
        Window_BattleLog.prototype.drawBackground = function () {
            ...
            var graphics = new PIXI.Graphics();
            graphics.beginFill(Saba.toPixiColor(color), this.backPaintOpacity() / 255);
            graphics.drawRect(rect.x, rect.y, rect.width, rect.height + 3);
            graphics.endFill();
            this._backSprite.addChild(graphics);
        };
        var _Window_BattleLog_prototype_clear = Window_BattleLog.prototype.clear;
        Window_BattleLog.prototype.clear = function () {
            _Window_BattleLog_prototype_clear.call(this);
            this._backSprite.removeChildren();
        }
    }

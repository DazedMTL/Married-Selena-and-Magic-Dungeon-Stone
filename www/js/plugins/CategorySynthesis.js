...
(function () {
    ...
    Window_Base.prototype.drawSynthesisItemName = function (item, num, x, y, width) {
        ...
        if (item) {
            ...
            if (num > 1) text += 'x' + num;
            if (item.etypeId > 0) {
                this.drawWeaponName(item, x, y, width);
            } else {
    ...
    Window_Recipe.prototype.price = function (item) {
        return getRecipePrice(item);
    }
    function getRecipePrice(item) {

        var recipe = DataManager.itemRecipe(item).recipe;

        var price = 0;
        for (var i = 0, max = recipe.length; i < max; i++) {
            var it = recipe[i];
            var id = parseInt(it[0].substring(1));
            var n = $gameParty.numItems($dataItems[id]);
            if (it[1] > n) {
                price += getMaterialPrice($dataItems[id]) * (it[1] - n);
            }
        }
        return price;
    }
    ...
    function getMaterialPrice(item) {
        var level = parseInt(item.meta['LV']);
        switch (level) {
            case 1: return 100;
            case 2: return 200;
            case 3: return 400;
    ...
    Window_Recipe.prototype.includes = function (item) {
        if ($gameParty.finishedItems) {
            for (var i = 0; i < $gameParty.finishedItems.length; i++) {
                var finished = $gameParty.finishedItems[i];
                if (finished.itemType == item._itemType && finished.itemId == item.id) {
                    return false;
                }
            }
        }
        ...
        return categories.indexOf(cate) >= 0;
    };

    Window_Recipe.prototype.isEnabled = function (item) {
        var result = (item && this.price(item) <= this._money);
            ...
            for (var i = 0, max = recipe.recipe.length; i < max; i++) {
                ...
                if (item) {
                    ...
                    if (DataManager.isSynthesisItem(item)) {
                        this.changePaintOpacity(this.isEnable([item, num]));
                        this.drawSynthesisStretchIcon(DataManager.decodeSynthesisItem(item).iconIndex, 22, y + 5, this.contentsWidth() - 80);
                    ...
                    if (!this.isEnable([item, num])) {
                        this.drawGold(item, num, this.contentsWidth() - 180, y);
                    }
    ...
    Window_SynthesisStatus.prototype.drawGold = function (item, num, x, y) {
        var id = parseInt(item.substring(1));

        var remain = num - this.numItems(item);
        var price = remain * getMaterialPrice($dataItems[id]);
        this.drawText('(' + price + ' G)', x, y, 200);
    };
    Window_SynthesisStatus.prototype.drawPossession = function (x, y) {
        ...
        this._actor = getActor(item);
        this.drawParamChange(x, y, item, 0);
        this.drawParamChange(x, y + l * 1, item, 2);
        this.drawParamChange(x, y + l * 2, item, 9);
        this.drawParamChange(x, y + l * 3, item, 8);
        this.drawParamChange(x, y + l * 4, item, 10);
        this.drawParamChange(x, y + l * 5, item, 11);
        this.drawParamChange(x, y + l * 6, item, 12);
    ...
    Window_SynthesisStatus.prototype.drawParamChange = function (x, y, item, paramId) {
        var tempActor = JsonEx.makeDeepCopy(this._actor);
        tempActor.forceChangeEquip(getSlot(item), item);

        var newValue = Math.round(tempActor.getStatusParam(paramId));
        var currentValue = this._actor.getStatusParam(paramId);
        var diffvalue = Math.round(newValue - currentValue);
        this.changeTextColor(this.systemColor());

        this.drawText(TextManager.param(paramId), x, y, 148);


        this.changeTextColor(this.normalColor());

        this.drawText(currentValue, x + 100, y, 48, 'right');
        this.drawText('→', x + 150, y, 48, 'left');

        this.changeTextColor(this.paramchangeTextColor(diffvalue));
        this.drawText(newValue, x + 160, y, 48, 'right');

        if (diffvalue > 0) {
            this.drawText('+' + diffvalue, x + 220, y, 48, 'left');
        }
    }

    var getSlot = function (item) {
        if (item.atypeId >= 1) {
            return item.etypeId - 1;
        }
        switch (item.wtypeId) {
            case 1:
            case 3:
    ...
    Scene_CategorySynthesis.prototype.createGoldWindow = function () {
        this._goldWindow = new Window_Gold(824, 0);
        this._goldWindow.y = this._helpWindow.y + this._helpWindow.height;
        this.addWindow(this._goldWindow);
        this._goldWindow.show();
    };
    Scene_CategorySynthesis.prototype.createConfirmWindow = function () {
    ...
    Scene_CategorySynthesis.prototype.onConfirmOk = function () {
        this.onSynthesisOk();
        this._confirmWindow.deactivate();
        this._confirmWindow.hide();
        var settings = synthesisSeSettings;
        var se = {
            name: synthesisSeName,
            volume: settings[0],
            pitch: settings[1],
            pan: settings[2]
        };
    ...
    Scene_CategorySynthesis.prototype.onConfirmCancel = function () {
        this.onSynthesisCancel();
        this._confirmWindow.deactivate();
        this._confirmWindow.hide();
    };
    ...
    Scene_CategorySynthesis.prototype.createRecipe = function (goods) {
        ...
        this._recipes = this._recipes.sort(function (a, b) {
            a = a[0];
            b = b[0];
            if (a[0] == 'A' && b[0] != 'A') {
                return 1;
            ...
            var id1 = a.substring(1);
            var id2 = b.substring(1);
    ...
    Scene_CategorySynthesis.prototype.createCategoryWindow = function () {
        ...
        if (!$gameTemp._callShopSynthesis) w = Graphics.boxWidth - 200;
    ...
    Scene_CategorySynthesis.prototype.createRecipeListWindow = function () {
        ...
        this._recipeWindow.setHandler('pagedown', this.onPagedown.bind(this));
        this._recipeWindow.setHandler('pageup', this.onPageup.bind(this));
    ...
    Scene_CategorySynthesis.prototype.onPageup = function () {
        var index = this._categoryWindow.index();
        index--;
        if (index < 0) {
        ...
        this._categoryWindow.select(index);
        this._categoryWindow.updateHelp();
        this._recipeWindow.activate();
    };
    Scene_CategorySynthesis.prototype.onPagedown = function () {
        var index = this._categoryWindow.index();
        index++;
        if ($gameSwitches.value(568)) {
            if (index > 3) {
    ...
    Scene_CategorySynthesis.prototype.doSynthesis = function () {
        ...
        var item = this._recipeWindow.item();
        $gameMedals.onMake();
        $gameParty.loseGold(getRecipePrice(item));
        for (var i = 0, max = this._listWindow._sets.length; i < max; i++) {
            var item2 = this._listWindow._sets[i];
            $gameParty.loseItem(item2, num);
        }
        ...
        if (!$gameParty.finishedItems) {
            $gameParty.finishedItems = [];
        }
        $gameParty.finishedItems.push(
            {
                itemId: item.id,
        ...
        getActor(item).forceChangeEquip(getSlot(item), item);
        this.createRecipe();
        this._recipeWindow._shopGoods = this._recipes;
        this._recipeWindow.refresh();
        this._recipeWindow.updateHelp();
    };
    var getActor = function (item) {
        ...
        if (categories.indexOf('シーフ') >= 0) {
            return $gameActors.actor(3);
        }
        if (categories.indexOf('プリースト') >= 0) {
            return $gameActors.actor(4);
        }
        if (categories.indexOf('ニンジャ') >= 0) {
            return $gameActors.actor(2);
        }
        return $gameActors.actor(15);
    };

...
(function ($) {
    ...
    // Parameters
    //-----------------------------------------------------------------------
    $.viewRadius = 3.8; //Number($.Params['View Distance']);
    $.fov = Number($.Params['Field Of View']) / 51.0;
    ...
    $.createCube = function (scene) {
        var cube = BABYLON.MeshBuilder.CreateBox('fple-cube', {
            size: 1.0
        }, scene);
        cube.setVerticesData(BABYLON.VertexBuffer.UVKind, getUvs());
        return cube;
    }
    $.createCube1 = function (scene) {
        var cube = BABYLON.MeshBuilder.CreateBox('fple-cube', {
            depth: 0.005,
            height: wallHeight,
            size: 1.0
...
(function () {
    ...
    MBS.FPLE.Map.prototype.refresh = function () {
        ...
        var tileB = width * height * 3;
        var tileC = width * height * 2;
        for (var x = 0; x < width; x++) {
            this._data[x] = [];
            for (var y = 0; y < height; y++) {
                this._data[x][y] = [data[y * width + x], data[y * width + x + tileB], data[y * width + x + tileC]];
            }
        }
    };
    MBS.FPLE.Map.prototype.terminate = function () {
        p('terminate')
        for (var i in this._cache.textures) {
            this._cache.textures[i].dispose();
        }
        for (var i in this._cache.materials) {
            this._cache.materials[i].dispose();
        }
    }
    //-----------------------------------------------------------------------
    ...
    MBS.FPLE.Map.prototype._updateEvents = function (scene) {
        ...
        this._events.forEach(function (sprite) {
            ...
            var direction;
            if (event.characterName().length == 0) {
                sprite.width = 0;
            ...
            sprite.width = 0.2;
            sprite.height = 0.2;
            sprite.position = new BABYLON.Vector3(-event._realX, MBS.FPLE.cameraHeight + 0.1, event._realY);
...
(function () {
    ...
    MBS.FPLE.Camera.prototype.initialize = function (scene) {
        ...
        this.rotation.z = 0; //-Math.PI; // Look up
        //this.rotation.x = Math.PI; // Look up
    };
    //-----------------------------------------------------------------------
    ...
    MBS.FPLE.Camera.prototype._updatePosition = function () {
        ...
        switch ($gamePlayer.direction()) {
            case 2:
                ...
                if (this._lastDirection == 6) {
                    yy = -1 * rate * (this._count) / total;
                    xx = +1 * rate * (total - this._count) / total;
                }
            ...
            case 4:
                ...
                if (this._lastDirection == 2) {
                    xx = -1 * rate * (this._count) / total;
                    yy = -1 * rate * (total - this._count) / total;
                }
        ...
        this.position = new BABYLON.Vector3(-$gamePlayer._realX + xx, 0.7, $gamePlayer._realY + yy);
    };
    //-----------------------------------------------------------------------
    ...
    MBS.FPLE.Camera.prototype._updateRotation = function () {
        ...
        if (Math.round(n * 10) / 10 === 0) {
            ...
            this.rotation.y += t * Math.PI / 180.0 * 4;
        }
...
(function () {
    ...
    Game_Player.prototype.moveStraight = function (d) {
        if (this.canPass(this.x, this.y, d)) {
            ...
            if (this.is3D() && $fple) {
                if (Saba.isGate(this.x, this.y, d)) {
                    AudioManager.playSe({
                        name: 'Open1',
                        volume: 90,
                        pitch: 100,
        ...
        if (!this.is3D() || !$fple) {
            ...
            if (Input.isTriggered('right')) {
                if (Input.isPressed('shift')) {
                    this._dashing = ConfigManager.alwaysDash;
                    this.moveRightShift();
                } else {
                    this.turnRight90();
                }
    ...
    Game_Player.prototype.is3D = function () {
...
(function () {
    ...
    MBS.FPLE.Scene.prototype._createLight = function () {
        ...
        //this._light = new BABYLON.SpotLight("spotLight", BABYLON.Vector3.Zero(), BABYLON.Vector3.Zero(), Math.PI / 3, 2, this);
        this._light.diffuse = new BABYLON.Color3.FromHexString(MBS.FPLE.lightColor);
        this._light.range = MBS.FPLE.viewRadius;
        this._light.intensity = 1.6;
    };
    //-----------------------------------------------------------------------
    // Updates the scene.
    //-----------------------------------------------------------------------
...
(function () {
    ...
    Scene_Map.prototype._createFPLE = function () {
        ...
        var loader = new BABYLON.AssetsManager($fple);
        /* BABYLON.SceneLoader.ImportMesh("", "./img/obj/", "skull.babylon", $fple, function (newMeshes) {
             try {
                /* p(newMeshes)
                 var m = newMeshes[1]
                 p(m.parent)
                 ...
                 m.setVerticesData(BABYLON.VertexBuffer.UVKind, getUvs());
                     p(333)*/
...
*/
    ...
    Scene_Map.prototype.terminateFple = function () {
        if (this.useFPLE()) {
            this._fple.terminate();
        }
        $fple = null;
...
(function () {
    ...
    Graphics._createFPLERenderer = function () {
        ...
        $babylon.loadingScreen = loadingScreen;
    };
    ...
    Graphics._updateFPLE = function () {
        ...
        this._fpleCanvas.height = this._height + cameraOffsetY;
        this._centerElement(this._fpleCanvas);
        this._fpleCanvas.style.top = -cameraOffsetY + 'px';
...
Game_CharacterBase.prototype.distancePerFrame = function () {
    return Math.pow(2, this.realMoveSpeed()) / 256 / 1.5;
};

SceneManager.clearBackgroundSprite = function () {
    if (this._backgroundSprite && this._backgroundSprite._texture) {
...
SceneManager.snapForBackground = function () {
    ...
    if (!$dataMap || !$dataMap.meta.fple || !$fple) {
    ...
    BABYLON.Tools.DumpFramebuffer(1024, 768 + cameraOffsetY, $fple.getEngine(), callback);
    this._backgroundSprite = sprite;
    var _destroy = this._backgroundSprite.destroy;
    this._backgroundSprite.destroy = function (param) {
        _destroy.call(self._backgroundSprite, true);
...
var _Scene_MenuBase_prototype_createBackground = Scene_MenuBase.prototype.createBackground;
...
var _Spriteset_Battle_prototype_createBackground = Spriteset_Battle.prototype.createBackground;
Spriteset_Battle.prototype.createBackground = function () {
    var s = SceneManager.backgroundSprite();
    if (s) {
        this._backgroundSprite = s;
        this._baseSprite.addChild(this._backgroundSprite);
    } else {
        _Spriteset_Battle_prototype_createBackground.call(this);
    }
};

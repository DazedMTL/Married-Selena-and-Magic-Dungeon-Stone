...
//-----------------------------------------------------------------------------
//  2017-12-20 - Version 2.1 - fixed a bug when trying to remove battle layers
//                             during the battle
...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.2.1) Create graphic layers for parallax mapping, fog, etc. View the 'Help' document for plugin commands and info.
 * 
...
(function () {
	...
	Spriteset_Battle.prototype.createLayerGraphics = function () {
		...
		// reset layer graphics if refreshing
		for (var layer in this.layerGraphics) {
			this._battleField.removeChild(this.layerGraphics[layer]);
		}
		this.layerGraphics = {};
	...
	Sprite_LayerGraphic.prototype.update = function () {
		...
		this._lValue = this.lValue();
		if (!this._lValue) return;

		this.updateGraphic();
		this.updatePosition();
	};

	Sprite_LayerGraphic.prototype.updateGraphic = function () {
		if (this.currentGraphic !== this._lValue.graphic) {
			this.createBitmap();
			this.currentGraphic = this._lValue.graphic;
		};
	};

	// Update Position
	Sprite_LayerGraphic.prototype.updatePosition = function () {
		this.z = this._lValue.z || 0;
		this.opacity = this._lValue.opacity || 0;
		this.blendMode = this._lValue.blend || 0;

		this.origin.x = 0 + this.displayX() * Galv.LG.tileSize + this._lValue.currentx + this.xOffset();
		this.origin.y = 0 + this.displayY() * Galv.LG.tileSize + this._lValue.currenty + this.yOffset();
		this._lValue.currentx += this._lValue.xspeed;
		this._lValue.currenty += this._lValue.yspeed;
	};

	Sprite_LayerGraphic.prototype.xOffset = function () {
		return this.displayX() * this._lValue.xshift;
	};

	Sprite_LayerGraphic.prototype.yOffset = function () {
		return this.displayY() * this._lValue.yshift;
	};
	...
	// Update Position
	Sprite_LayerGraphicS.prototype.updateGraphic = function () {
		if (this.currentGraphic !== this._lValue.graphic) {
			this.createBitmap(this._lValue);
			this.currentGraphic = this._lValue.graphic;
		};
	};
